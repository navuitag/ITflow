...
export function submitAnswer(answer, question, errorPatterns) {
  ...
  updateState((state) => {
    ...
    state.dailyQuest.progress = Math.min(
      state.dailyQuest.target,
      state.dailyQuest.progress + (correct ? 1 : 0)
    );
    state.answers.push({
    ...
    state.skillMastery[question.skill] = Math.max(
      0,
      Math.min(100, currentMastery + (correct ? 12 : -6))
    );
  ...
  return { correct, error, xp: xpForAnswer(correct) };
}
